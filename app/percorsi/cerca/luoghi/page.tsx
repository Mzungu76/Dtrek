...
function parseMetaTypeParam(v: string | null): SearchMetaType {
  return v === 'sito' ? 'sito' : 'borgo_citta'
}

/**
 * Cerca/crea una Meta Borgo-Città o Sito (piano §17/§25 — Blocco C/D già costruiti). Riusa
 * /api/meta-search (searchBorghi/searchSiti, lib/metaSearch) per la ricerca e
 * metaSearchResultToPlannedHike + savePlanned per la creazione — nessuna nuova logica di
 * ricerca/creazione qui, solo la UI. 'sentiero' resta volutamente fuori: quel flusso è
 * /upload?tab=gpx, invariato (piano §48.3/§18).
 *
 * Spostata da /percorsi/cerca a /percorsi/cerca/luoghi (docs/piano-ricerca-mete.md, Fase 4):
 * quella rotta è ora l'hub unico di ricerca delle Mete (tutte le tipologie), questa resta lo
 * scaffale "Borghi/Città e Siti" aperto da lì — logica invariata, solo `?tipo=` in più per
 * arrivare con la tipologia già scelta invece di dover ritoccare il tab.
 *
 * `useSearchParams` richiede un confine Suspense (stesso pattern di app/upload/page.tsx).
 */
export default function CercaLuoghiPage() {
  return (
    <Suspense fallback={null}>
      <CercaLuoghiPageInner />
    </Suspense>
  )
}

function CercaLuoghiPageInner() {
  const searchParams = useSearchParams()
  const [metaType, setMetaType] = useState<SearchMetaType>(() => parseMetaTypeParam(searchParams.get('tipo')))
