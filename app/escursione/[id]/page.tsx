...
import { getActivityById, updateActivityMeta, deleteActivity, type StoredActivity } from '@/lib/store'
...
export default function EscursionePage() {
  ...
  const saveTitle = () => {
    updateActivityMeta(id, { title: titleVal })
    setActivity(prev => prev ? { ...prev, title: titleVal } : prev)
    setEditTitle(false)
  }

  const saveNotes = () => {
    updateActivityMeta(id, { userNotes: notesVal })
    setActivity(prev => prev ? { ...prev, userNotes: notesVal } : prev)
    setEditNotes(false)
  }
  ...
  const addTag = () => {
    ...
    updateActivityMeta(id, { tags })
    setActivity(prev => prev ? { ...prev, tags } : prev)
    setTagInput('')
  }

  const removeTag = (tag: string) => {
    const tags = (activity.tags ?? []).filter(t => t !== tag)
    updateActivityMeta(id, { tags })
    setActivity(prev => prev ? { ...prev, tags } : prev)
  }

  const handleDelete = () => {
    if (confirm('Eliminare questa escursione dal diario?')) {
      deleteActivity(id)
      router.push('/')
    }
  }
        ...
        <div className="max-w-6xl mx-auto px-4 py-6">
            ...
            <div>
              {editTitle ? (
                <div className="flex items-center gap-2">
                  ...
                  />
                  <button onClick={saveTitle}><Check className="w-5 h-5 text-forest-300 hover:text-white" /></button>
              ...
              {activity.device && (
                <p className="text-forest-400 text-xs mt-0.5">📱 {activity.device}</p>
              )}
            </div>

            <div className="flex gap-2 flex-wrap">
              <button
                onClick={() => exportActivityToExcel(activity)}
              ...
              <button
                onClick={() => exportActivityToDoc(activity)}
                className="flex items-center gap-1.5 px-3 py-2 bg-forest-700 hover:bg-forest-600 rounded-lg text-sm transition-colors"
              >
          ...
          <div className="flex items-center gap-2 mt-4 flex-wrap">
            {(activity.tags ?? []).map(tag => (
              <span
                key={tag}
                className="flex items-center gap-1 bg-forest-700 text-forest-200 rounded-full px-3 py-0.5 text-xs"
              >
                {tag}
          ...
          {editNotes ? (
            <div>
              ...
              <div className="flex gap-2 mt-2">
                <button
                  onClick={saveNotes}
                  className="px-4 py-1.5 bg-forest-600 text-white rounded-lg text-sm hover:bg-forest-700 transition-colors"
                >
                  Salva
                </button>
                <button
            ...
            <p className={`text-sm leading-relaxed ${activity.userNotes ? 'text-stone-600' : 'text-stone-400 italic'}`}>
              {activity.userNotes || 'Nessuna nota. Clicca "Modifica" per aggiungere appunti sull\'escursione.'}
            </p>
          )}
        </section>
      </main>
