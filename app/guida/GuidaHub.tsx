...
export default function GuidaHub({ id }: { id?: string }) {
    ...
    return (
      <>
        {hike.cachedTrailScore != null && (
          <button onClick={onTap} className={`${glassChip} text-[11px] font-semibold px-2.5 py-1.5`}>CTS {Math.round(hike.cachedTrailScore)}</button>
        )}
        {safetyScore && (
          <button onClick={onTap} className="px-2.5 py-1.5 rounded-full text-white text-[11px] font-bold border border-white/10 backdrop-blur-md" style={{ backgroundColor: safetyScore.color }}>Sicurezza {safetyScore.label}</button>
        )}
        {hike.cachedBeautyScore && (
          <button onClick={onTap} className="px-2.5 py-1.5 rounded-full text-white text-[11px] font-bold border border-white/10 backdrop-blur-md" style={{ backgroundColor: hike.cachedBeautyScore.color }}>Bellezza {hike.cachedBeautyScore.gradeLabel}</button>
        )}
      </>
    )
  }
  ...
  const renderSection = (section: SectionKind, item: RouteHubItem, onClose: () => void) => {
    ...
    if (section === 'dati') return (
      <SectionOverlay title="Dati & punteggi" onClose={onClose} on3D={open3D(onClose)} mapHeaderActions={poiToggleChip}>
    ...
    if (section === 'meteo') return (
      <SectionOverlay title="Meteo" onClose={onClose} on3D={open3D(onClose)} mapHeaderActions={poiToggleChip}>
    ...
    if (section === 'natura') return (
      <SectionOverlay title="Natura" onClose={onClose} on3D={open3D(onClose)} mapHeaderActions={poiToggleChip}>
        <div className="px-4 py-4 space-y-5">
    ...
    if (section === 'poi') return (
      <SectionOverlay title="Punti di interesse" onClose={onClose} on3D={open3D(onClose)} mapHeaderActions={poiToggleChip} scrollRef={poiCenter.containerRef}>
    ...
    if (section === 'sicurezza') {
      ...
      return (
        <SectionOverlay title="Sicurezza & segnalazioni" onClose={onClose} on3D={open3D(onClose)} mapHeaderActions={poiToggleChip} scrollRef={sicurezzaCenter.containerRef}>
          <div className="px-4 py-4 space-y-5">
    ...
    if (section === 'featured') {
      ...
      return (
        <SectionOverlay title="Guida Turistica" onClose={onClose} on3D={open3D(onClose)} mapHeaderActions={poiToggleChip} scrollRef={guideCenter.containerRef}>
    ...
    // strumenti
    return (
      <SectionOverlay title="Strumenti" onClose={onClose} on3D={open3D(onClose)} mapHeaderActions={poiToggleChip}>
