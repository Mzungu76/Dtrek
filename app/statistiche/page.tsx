...
import { FileSpreadsheet, TrendingUp, Mountain, Heart, Route, Flame, Clock, Loader2 } from 'lucide-react'
...
export default function StatistichePage() {
  ...
  const monthlyData = activities
    .reduce((acc, a) => {
      ...
      if (ex) { ex.km += a.distanceMeters / 1000; ex.count++ }
      else acc.push({ month, km: a.distanceMeters / 1000, count: 1 })
      return acc
  ...
  const fcData = [...activities]
    ...
    .map(a => ({ data: format(new Date(a.startTime), 'dd/MM'), fc: a.avgHeartRate }))

  // exportAllActivitiesToExcel accetta StoredActivity[] ma ActivityMeta ha tutti i campi necessari
  const handleExport = () => exportAllActivitiesToExcel(activities as any)
                ...
                <div className="h-56">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={monthlyData} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
                      ...
                      <Tooltip formatter={(v: number) => [`${v} km`, 'Distanza']} contentStyle={{ borderRadius: 8, border: '1px solid #e8e4dc', fontSize: 13 }} />
                ...
                <div className="h-56">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={fcData} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
                      ...
                      <Tooltip formatter={(v: number) => [`${v} bpm`, 'FC media']} contentStyle={{ borderRadius: 8, border: '1px solid #e8e4dc', fontSize: 13 }} />
                  ...
                  <tbody className="divide-y divide-stone-100">
                    {activities.map(a => (
                      <tr key={a.id} className="hover:bg-stone-50 cursor-pointer transition-colors"
                        ...
                        <td className="px-4 py-3 font-medium text-stone-700">{a.title ?? 'Escursione'}</td>
