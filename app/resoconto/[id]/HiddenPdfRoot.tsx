        ...
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '20px 32px' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
            {[
              ...
              <span key={v} style={{ fontSize: 10, padding: '2px 8px', borderRadius: 12, background: 'rgba(255,255,255,0.2)', color: 'white', fontWeight: 600, fontFamily: FONT.barlow }}>{v}</span>
            ))}
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: '32px 32px 0' }}>
                ...
                {sectionPhoto && slot > 0 && (
                  <div style={{ float: 'right', marginLeft: 12, marginBottom: 8, width: 120 }}>
                    <div style={{ position: 'relative' }}>
                      ...
                      <span style={{ position: 'absolute', top: 4, left: 4, width: 16, height: 16, background: '#f59e0b', color: 'white', borderRadius: '50%', fontSize: 8, fontWeight: 'bold', fontFamily: FONT.barlow, textAlign: 'center', lineHeight: '16px', display: 'block', boxSizing: 'border-box' }}>{slot+1}</span>
                    </div>
                    <p style={{ fontFamily: FONT.lora, fontSize: 8, color: '#78716c', textAlign: 'center', marginTop: 3, fontStyle: 'italic' }}>{sectionPhoto.caption}</p>
                  </div>
                )}
                {section.body.split(/\n\n+/).map((p, j) => (
                  <p key={j} style={{ fontFamily: FONT.lora, fontSize: 11, lineHeight: 1.7, color: '#374151', margin: '0 0 8px' }}>{p.replace(/\[curiosita\]|\[\/curiosita\]/g, '').trim()}</p>
                ))}
              </div>
            </div>
          )
        ...
        {photos.length > 0 && (
          <div className="pdf-block" style={{ borderTop: '1px solid #e5e7eb', paddingTop: 16, marginTop: 8 }}>
            <h3 style={{ fontFamily: FONT.barlow, fontSize: 9, fontWeight: 700, letterSpacing: 3, textTransform: 'uppercase', color: '#9ca3af', marginBottom: 12 }}>Documentazione fotografica</h3>
              ...
              {photos.map((ph, i) => (
                <div key={ph.id} className="pdf-block">
                  <div style={{ position: 'relative' }}>
                    ...
                    <span style={{ position: 'absolute', top: 4, left: 4, width: 16, height: 16, background: '#f59e0b', color: 'white', borderRadius: '50%', fontSize: 7, fontWeight: 'bold', fontFamily: FONT.barlow, textAlign: 'center', lineHeight: '16px', display: 'block', boxSizing: 'border-box', border: '1px solid white' }}>{i+1}</span>
                  </div>
                  {ph.caption && <p style={{ fontFamily: FONT.lora, fontSize: 8, color: '#78716c', textAlign: 'center', marginTop: 3, fontStyle: 'italic' }}>{i+1}. {ph.caption}</p>}
                </div>
              ))}
            </div>
          </div>
