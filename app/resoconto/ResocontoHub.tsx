...
export default function ResocontoHub({ id }: { id?: string }) {
  ...
  const renderStageMap = (item: RouteHubItem) => {
    ...
    return (
      <MapView trackPoints={activity.trackPoints} height="100%" interactive
  ...
  const renderAltimetryMap = (item: RouteHubItem, activeIndex: number | null) => {
    if (!activity || item.id !== activity.id || !hasGps) return <div className="absolute inset-0 bg-[#0b1a24]" />
    return <MapView trackPoints={activity.trackPoints} height="100%" interactive={false} activeIndex={activeIndex} />
  }
  ...
  const renderAltimetryChart = (item: RouteHubItem, onHover: (i: number | null) => void, onActivePoint: (d: { alt: number; kmNum: number } | null) => void) => {
    if (!activity || item.id !== activity.id || !activity.trackPoints.length) {
      return <div className="h-full flex items-center justify-center text-stone-400 text-sm">Dati altimetrici non disponibili</div>
    }
    return <ElevationProfileChart trackPoints={activity.trackPoints} onHover={onHover} onActivePoint={onActivePoint} />
  }
  ...
  const renderPopup = (popup: PopupKind, item: RouteHubItem) => {
    if (!activity || item.id !== activity.id) return <div className="py-10 text-center text-sm text-stone-400">Caricamento…</div>
    ...
    if (popup === 'poi') return (
      <div className="space-y-5">
        ...
        {pois.length > 0 && <p className="text-xs text-stone-400">{pois.length} punti di interesse trovati lungo il tracciato</p>}
        {hasGps && (
    ...
    return (
      <div className="space-y-1">
        ...
        <div className="pt-1 mt-1 border-t border-stone-100 space-y-1">
          ...
          <button onClick={() => router.push(`/resoconto/${encodeURIComponent(activity.id)}/leggi`)} className="w-full flex items-center gap-3 px-2 py-3 rounded-xl hover:bg-stone-50 transition-colors text-left">
            <BookOpen className="w-4 h-4 text-stone-400" /> <span className="text-sm font-medium text-stone-700">Leggi racconto</span>
          </button>
        </div>
