...
export default function RecoSuggestedRow({ cards }: { cards: RecommendationCard[] }) {
  return (
    // pt-3 in più (oltre a pb-1) — spazio per il nastro che sporge sopra ogni card (Taccuino
    // Botanico, TornFrame "card").
    <div className="flex gap-2.5 overflow-x-auto pt-3 pb-1 -mx-4 px-4" style={{ scrollbarWidth: 'none' }}>
      {cards.map(card => {
        ...
        return (
          ...
          <TornFrame key={card.id} size="card" variant={tornVariant(card.id)} className="shrink-0 w-[170px]">
            <Link
