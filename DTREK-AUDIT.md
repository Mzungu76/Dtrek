...
| 26 | ✅ **Risolto** — Confermato: `OnboardingWizard.tsx` copre solo il profilo escursionista, nessun punto della UI di navigazione (SOS, vie d'uscita, layer, colori POI) veniva mai spiegato prima che un escursionista lo scoprisse da solo sul sentiero. Nuovo `NavOnboardingSheet.tsx`: riepilogo compatto (non un tutorial passo-passo sovrapposto ai controlli reali, troppo fragile da mantenere sincronizzato) con le stesse icone/colori dei controlli veri — Emergenza (scudo rosso), Vie d'uscita (sempre raggiungibile), Condividi posizione, Layer mappa, Colori POI — mostrato **una sola volta per dispositivo**, all'avvio della prima navigazione attiva mai fatta (`lib/navigation/navOnboardingPref.ts`, localStorage semplice come `highContrastPref.ts` — non un dato utente da sincronizzare tra dispositivi), **prima** che possa servire davvero, non durante un'emergenza. Riapribile in ogni momento da un nuovo pulsante "?" nella rotaia azioni. | `lib/navigation/navOnboardingPref.ts`, `components/navigation/NavOnboardingSheet.tsx`, `components/navigation/ActiveNavigationView.tsx` | `tsc --noEmit` pulito; suite `vitest` completa (190/190) verde (nessun nuovo test — puro contenuto/wiring UI, nessuna logica pura da isolare, stesso trattamento già dato a `highContrastPref.ts` in P1 #20); build di produzione verificata (trovato e corretto un apostrofo non escapato in JSX, `react/no-unescaped-entities`); verifica visiva reale non eseguibile da questo sandbox (nessun browser/dispositivo). |
