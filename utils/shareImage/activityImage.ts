...
import { FONT, DARK, makeCanvas, isTall, drawDarkBg, rr, drawCard, drawLogo, drawElevationProfile, fitText, drawRouteAbstract, loadImageEl, type ShareFormat } from './canvasHelpers'
...
/**
 * Disegna la scheda "mappa a tutto campo" (mappa MapLibre + gradienti + titolo + statistiche) su
 * un canvas già pronto. Estratta da `generateActivityImage` perché il carosello (Fase 2.2) la
 * riusa identica per la copertina — la resa MapLibre, la parte costosa, avviene una sola volta e
 * il resto è puro disegno 2D.
 */
export async function drawActivityMapCard(
  ctx: CanvasRenderingContext2D,
  activity: ActivityMeta,
  opts: ActivityShareOpts,
  fmt: ShareFormat,
  w: number, h: number,
  /** Mappa già resa (es. dal carosello, che la riusa anche per la scheda di chiusura senza pagare
   *  due volte il costo di MapLibre). Se assente viene resa qui. */
  preRenderedMapImg?: HTMLImageElement,
): Promise<void> {
  ...
  const topH = fmt === '9:16' ? 300 : fmt === '1:1' ? 210 : fmt === '4:5' ? 240 : 175    // gradient header height
  const botH = showProfileBand
    ? (fmt === '9:16' ? 460 : fmt === '1:1' ? 330 : fmt === '4:5' ? 370 : 250)
    : (fmt === '9:16' ? 300 : fmt === '1:1' ? 190 : fmt === '4:5' ? 220 : 160)

  // 1. Mappa MapLibre (rilievo, sentieri, traccia con marker inizio/fine, attribuzione incisa),
  //    resa alla dimensione esatta del canvas e poi disegnata a tutto campo.
  const mapImg = preRenderedMapImg ?? await loadImageEl(await renderRouteMap({
    polylines: [polyline],
    width: w, height: h,
    pixelRatio: 2,
    markers: [
  ...
  // 2. Top gradient — stronger to ensure white text reads over the outdoor basemap's light areas
  ...
  // 3. Bottom gradient — stronger for the same reason
  ...
  const titleSz = fmt === '9:16' ? 74 : fmt === '1:1' ? 62 : fmt === '4:5' ? 66 : 48
  ...
  if (opts.showDate) {
    const dateSz = fmt === '9:16' ? 30 : fmt === '1:1' ? 26 : fmt === '4:5' ? 28 : 22
  ...
  const valY    = h - (fmt === '9:16' ? 110 : fmt === '1:1' ? 52 : fmt === '4:5' ? 70 : 44)
  const valSz   = fmt === '9:16' ? 42 : fmt === '1:1' ? 34 : fmt === '4:5' ? 36 : 28

  // 5a. Elevation profile band — sits just above the stats row
  if (showProfileBand) {
    const profH = fmt === '9:16' ? 165 : fmt === '1:1' ? 115 : fmt === '4:5' ? 130 : 82
  ...
  if (pillData.length > 0) {
    ...
    const lblY    = valY + (fmt === '9:16' ? 32 : fmt === '1:1' ? 28 : fmt === '4:5' ? 30 : 24)
    const lblSz   = fmt === '9:16' ? 15 : fmt === '1:1' ? 13 : fmt === '4:5' ? 14 : 11
...
export async function generateActivityImage(
  ...
  // ── Full-bleed map ────────────────────────────────────────────────────────
  if (useMap) {
    await drawActivityMapCard(ctx, activity, opts, fmt, w, h)
